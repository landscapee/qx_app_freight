...
public class LoadAndUnloadTodoModel extends BaseModel implements LoadAndUnloadTodoContract.loadAndUnloadTodoModel {
    ...
    @Override
    public void startClearTask(TaskClearEntity taskClearEntity, IResultLisenter lisenter) {
        Disposable subscription = UpdateRepository.getInstance().startClearTask(taskClearEntity)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(lisenter::onSuccess, throwable -> {
                    lisenter.onFail(throwable.getMessage());
