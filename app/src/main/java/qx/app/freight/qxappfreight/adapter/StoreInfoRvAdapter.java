...
public class StoreInfoRvAdapter extends BaseQuickAdapter<MainListBean.DataBean.SingeDataBean, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, MainListBean.DataBean.SingeDataBean item) {
        helper.setText(R.id.tv_goods_order, "001").setText(R.id.tv_goods_number, String.format(mContext.getString(R.string.format_store_number_goods_info), item.getSingleNumber(), item.getSingleWeight(), (float) item.getSingleVolume()));
