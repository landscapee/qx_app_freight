...
public abstract class TCPClientBase implements Runnable {
    ...
    private void ackServer(Message m) throws Exception {
        ...
        if (m.getCmd() == Message.CMD_0x20) {
            try {
                ...
                byte[] data = m.getData();
                String msgstrID = m.GetMsgID();
                long msgID = Long.parseLong(msgstrID);
                ByteBuffer.wrap(buffer).put((byte) Message.version).put((byte) 2).put((byte) Message.CMD_0x20).put(uuid).putLong(msgID);
    ...
    private void send(byte[] data1) throws Exception {
        if (data1 == null) {
            return;
        }
